...
cv::GStreamingCompiled ClassificationModel::compile_cv_graph() const
{
    ...
    cv::GArray<int> ids;
    cv::GArray<float> cfs;
    std::tie(ids, cfs) = cv::gapi::streaming::parse_class(nn, sz);
...
void ClassificationModel::preview(const cv::Mat &rgb, const std::vector<int> &labels, const std::vector<float> &confidences) const
{
    ...
    {
        // Draw a label with its confidence score. Get a new color for each item we detect,
        ...
        auto origin = cv::Point(0, i * 20) + cv::Point(3, 20);
